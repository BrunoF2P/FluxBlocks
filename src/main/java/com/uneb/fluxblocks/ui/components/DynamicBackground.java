...
public class DynamicBackground extends Pane {
    ...
    private void setupResizeListener() {
        canvas.widthProperty().addListener((obs, oldVal, newVal) -> {
            if (newVal.doubleValue() > 0) {
                calculateBlockSize(newVal.doubleValue(), canvas.getHeight());
                redraw();
            }
        });
        ...
        canvas.heightProperty().addListener((obs, oldVal, newVal) -> {
            if (newVal.doubleValue() > 0) {
                calculateBlockSize(canvas.getWidth(), newVal.doubleValue());
                redraw();
            }
        });
    }
