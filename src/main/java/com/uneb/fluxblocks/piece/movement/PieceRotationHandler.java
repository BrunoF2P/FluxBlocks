...
public class PieceRotationHandler {
     ...
     */
    public boolean rotate(BlockShape piece) {
    ...
    private boolean isRotationOnCooldown() {
        ...
        double requiredDelay = isFirstRotate ? GameConfig.ROTATE_INITIAL_DELAY : GameConfig.ROTATE_REPEAT_DELAY;
     ...
     */
    private RotationResult attemptRotationWithWallKicks(BlockShape piece) {
     ...
     * @return Resultado da tentativa de rotação
     */
    private RotationResult tryWallKickMoves(BlockShape piece, int originalX, int originalY) {
        piece.move(1, 0);
        if (collisionDetector.isValidPosition(piece)) {
            return new RotationResult(true, originalX, originalY);
        }
